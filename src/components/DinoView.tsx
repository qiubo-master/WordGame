...
import { generateLevels } from '../engine/levels'
...
export function DinoView({levelId,onExit}:Props){
  ...
  const level=useMemo(()=>meta?generateLevels({meta,words:bookWords},settings).find(x=>x.id===levelId):undefined,[meta,bookWords,settings,levelId]);const words=useMemo(()=>{const ix=new Map(bookWords.map(w=>[w.id,w]));return level?.wordIds.map(id=>ix.get(id)).filter((w):w is Word=>!!w)??[]},[bookWords,level])
  ...
  useEffect(()=>{if((phase!=='won'&&phase!=='lost')||recorded.current||!level||!meta)return;recorded.current=true;const passed=phase==='won';recordGameResult({levelId:level.id,bookId:meta.id,score,correct,wrong,maxCombo,durationMs:Date.now()-startedAt.current,coinsEarned:coinsForGame(settings,score,wrong,passed),passed,playedAt:Date.now(),targetScore:level.targetScore})},[phase,level,meta,score,correct,wrong,maxCombo,settings,recordGameResult])
  ...
  if(phase==='won'||phase==='lost'){const won=phase==='won';const coins=coinsForGame(settings,score,wrong,won);return <div className={`dino-result ${won?'won':'lost'}`}><div>{won?'🦾🏆':'🦖💥'}</div><h2>{won?'恐龙 Boss 被击败！':'军营被恐龙摧毁'}</h2><p>{score} 分 · 答对 {correct} · +{coins} 金币</p><div className="btn-row"><button className="btn ghost" onClick={onExit}>返回</button><button className="btn" onClick={begin}>再次挑战</button></div></div>}
