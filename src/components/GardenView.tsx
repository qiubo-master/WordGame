...
export function GardenView({levelId,onExit}:Props){
  ...
  const begin=()=>{unlockAudio();consumeEquipped();recorded.current=false;startedAt.current=Date.now();setSun(50+effects.extraTime*5);setBaseHp(settings.gardenBaseHp+effects.shield*20);setPlants([]);setZombies([]);setScore(0);setCorrect(0);setWrong(0);setCombo(0);setMaxCombo(0);setWave(0);setPicked(null);setPhase('playing')}
  ...
  if(phase==='won'||phase==='lost'){const won=phase==='won';const coins=coinsForGame(settings,score,wrong,won);return <div className={`garden-result ${won?'won':'lost'}`}><div className="garden-result-icon">{won?'🌻🏆':'🧟💥'}</div><h2>{won?'花园保卫成功！':'僵尸突破防线'}</h2><p>{score} 分 · 答对 {correct} · +{coins} 金币</p><div className="btn-row"><button className="btn ghost" onClick={onExit}>返回</button><button className="btn" onClick={begin}>再来一局</button></div></div>}
